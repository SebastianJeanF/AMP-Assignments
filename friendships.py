...
def no_imaginary(people: list[str], likes: list[tuple[str, str]]) -> bool:
    ...
    set_of_people = set(people)
    for (liker, likee) in likes:
        if(liker not in set_of_people or likee not in set_of_people):
            return False
...
def friends(likes: list[tuple[str, str]]) -> bool:
    ...
    # Check if everyone who likes someone is liked by that person
    for like in likes:
        liker, likee = like
        if (likee, liker) not in like_set:
            return False
...
def self_love(likes: list[tuple[str, str]]) -> list[tuple[str, str]]:
    ...
    non_selfish_people = [(liker, likee) for liker, likee in likes if liker != likee]
    return non_selfish_people 
...
def antisocial_social_club(people: list[str], likes: list[tuple[str, str]]) -> list[str]:
    ...
    """
    def likes_anyone(person: str, likes: list[tuple[str, str]]) -> bool:
        for like in likes:
            if like[0] == person:
                return True
        return False

    antisocial_people = [person for person in people if not likes_anyone(person, likes)]

    return antisocial_people
...
def count_friends(people: list[str], likes: list[tuple[str, str]]):
    ...
    friend_counts = [0] * len(people)
    like_dict = {}

    for like in likes:
        liker, likee = like
        if liker not in like_dict:
            like_dict[liker] = set()
        like_dict[liker].add(likee)

    for i, person in enumerate(people):
        if person in like_dict:
            for friend in like_dict[person]:
                if friend in like_dict and person in like_dict[friend]:
                    friend_counts[i] += 1
...
if __name__ == "__main__":
    ...
    """
    people = ["Joshua", "Sebastian", "Sam", "David", "Dylan", 
    "Angela", "Sai", "Dean", "Jiayi", "Echkart", "Jesus"]
    # Test for no_imaginary
    assert no_imaginary(people, [("hi", "hi")]) == False, "Ruh-roh"
    assert no_imaginary(people, [("Sebastian", "Echkart"), ("Joshua", "Echkart"), ("Sebastian", "Echkart")]) == True, "Ruh-roh"
    # Test for friends
    
    assert friends([("Bill", "Ted")]) == False, "Ruh-roh"
    assert friends([("Bill", "Ted"), ("Ted", "Bill")]) == True, "Ruh-roh"
    assert friends([("Bill", "Ted"),("Sarah", "Bill"), ("Ted", "Bill"), ("Bill", "Sarah") ]) == True, "Ruh-roh"

    # Test for self_love
    
    assert self_love([("me", "you"), ("you", "me")]).sort() == [("me", "you"), ("you", "me")].sort(), "Ruh-roh"

    # Test for antisocial_social_club
    assert antisocial_social_club(people, [("me", "you"), ("Sebastian", "me")]).sort() ==["Joshua", "Sam", "David", "Dylan", 
    "Angela", "Sai", "Dean", "Jiayi", "Echkart", "Jesus"].sort(), "Ruh-roh"
    

    # Test for count_friends


    assert count_friends(["Bill","Ted","Ana"], [("Bill","Ted"), ("Ted", "Bill"), ("Ana", "Ted"), ("Ted", "Ana")]) == [1, 2, 1], "Ruh-roh"